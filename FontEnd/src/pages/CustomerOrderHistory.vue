...
function statusLabel(s: OrderStatus) {
  const m: Record<OrderStatus, string> = { pending: 'Đang chờ', preparing: 'Đang pha chế', ready: 'Sẵn sàng', done: 'Hoàn thành', cancelled: 'Đã huỷ' }
  return m[s]
}
...
function statusBg(s: OrderStatus) {
  const m: Record<OrderStatus, string> = { pending: 'bg-amber-50', preparing: 'bg-blue-50', ready: 'bg-emerald-50', done: 'bg-green-50', cancelled: 'bg-red-50' }
  return m[s]
}
function statusIcon(s: OrderStatus) {
  return { pending: Clock, preparing: Loader2, ready: CheckCheck, done: CheckCheck, cancelled: XCircle }[s]
}
function statusIconColor(s: OrderStatus) {
  return { pending: 'text-amber-500', preparing: 'text-blue-500', ready: 'text-emerald-500', done: 'text-green-600', cancelled: 'text-red-500' }[s]
}
